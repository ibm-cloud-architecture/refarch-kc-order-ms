...
We assume you are now familiar with the event storming methodology as presented in [this note](https://ibm-cloud-architecture.github.io/refarch-eda/methodology/eventstorming/). And you have some basic knowledge of domain driven design, from reading books like [Eric Evans's "Domain Driven Design: Tackling Complexity in the Heart of Software" book]( https://learning.oreilly.com/library/view/domain-driven-design-tackling) and reading our summary in [this note](https://ibm-cloud-architecture.github.io/refarch-eda/methodology/ddd/)
...
* The pickup location where empty refrigerator container will be loaded
* The delivery location where the reefer is to be delivered to (we expect this to be in a remote country requiring a sea voyage or itinerary)
...
In Domain-driven design, an aggregate groups related objects as a unique entity. One object is the aggregate root. It ensures the integrity of the whole. Here the root is the Shipping Order. The product to ship is part of the aggregate. The Shipping Order is what we will persist in one unique transaction.
...
It is important to assess the main business entity's life cycle to define states and events to be produced as part of the state transition. The following diagram illustrates a potential life cycle for the shipping order aggregate:
