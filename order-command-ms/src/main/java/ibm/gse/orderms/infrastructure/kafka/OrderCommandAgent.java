...
/**
 * OrderCommandAgent listens to create, update and reject order commands that
 * arrives to the order-commands topic. After creating, updating or rejecting
 * an order, an event representing those actions will get published into the
 * orders topic as a result so that any other components from the overall
 * application that is interested on orders can be aware of the changes.
 *
 * It uses the Shipping Order Repository to save and update the orders
 * accordingly with the commands received. This repository is an in-memory Map
 * for simplicity, but it will work the same with an external database
 * 
 * We have implemented the consume-transform-produce loop pattern in Event
 * Driven Architectures so that every command is processed exactly once.
 */
@ApplicationScoped
public class OrderCommandAgent implements EventListenerTransactional {
	...
	public OrderCommandAgent() {
		...
		// messages before the transaction completes.
	...
	public OrderCommandAgent(ShippingOrderRepository repo, KafkaConsumer<String, String> kafka,
			EventEmitterTransactional oee, EventEmitterTransactional eep, KafkaInfrastructureConfig config) {
	...
	/**
	 * Get n records from the order-command topic, calculates the offset to be
	 * committed as part of the transaction and initiates the processing of the
	 * command.
	 * 
	...
	public void poll() {
		...
		// The kafka consumer poll api ensures liveness. The consumer sends periodic
	 ...
	 * others services responded with a Voyage and a Container assignment events.
	 * When the 'order created` event is generated, the read from command topic can
	 * be committed. It commits the offset only when both save to the repository and
	...
	private void processOrderCreation(OrderCommandEvent commandEvent,
		...
		try {
			...
			// This action MUST support repetition given that the order creation process
			// might get repeated
			// if all the actions to perform do not succeed.
			orderRepository.addOrUpdateNewShippingOrder(shippingOrder);
		} catch (OrderCreationException e) {
			// need other components to fix this save operation: CLI / human or automatic
			// process
			generateErrorEvent(shippingOrder, offsetToCommit);
			return;
		}
		// Create the event to be sent to the orders topic
		OrderEvent orderCreatedEvent = new OrderEvent(new Date().getTime(), OrderEvent.ORDER_CREATED_TYPE,
				schemaVersion, shippingOrder.toShippingOrderPayload());
		try {
	...
	private void processOrderUpdate(OrderCommandEvent commandEvent,
		...
		try {
			...
			if (oco.isPresent()) {
				...
				// This action MUST support repetition given that the order update process might
				// get repeated
				// if all the actions to perform do not succeed.
				orderRepository.updateShippingOrder(shippingOrder);
				// Create the event to be sent to the orders topic
				OrderEvent orderUpdateEvent = new OrderEvent(new Date().getTime(), OrderEvent.ORDER_UPDATED_TYPE,
						schemaVersion, shippingOrder.toShippingOrderPayload());
				try {
					// Emit the event and consumer offsets as a transaction
					orderEventProducer.emitWithOffsets(orderUpdateEvent, offsetToCommit,
	 ...
	 * @param commandEvent
	 */
	private void processOrderCancellation(OrderCommandEvent commandEvent,
			Map<TopicPartition, OffsetAndMetadata> offsetToCommit) {
		ShippingOrder shippingOrder = new ShippingOrder(commandEvent.getPayload());
		String orderID = shippingOrder.getOrderID();
		try {
			...
			if (oco.isPresent()) {
				...
				orderRepository.updateShippingOrder(shippingOrder);
				// Create the event to be sent to the orders topic
				OrderCancelledEvent orderCancelledEvent = new OrderCancelledEvent(new Date().getTime(), schemaVersion,
						shippingOrder.toOrderCancelAndRejectPayload("Cancel order command received"));
				try {
					// Emit the event and consumer offsets as a transaction
					orderEventProducer.emitWithOffsets(orderCancelledEvent, offsetToCommit,
							"ordercmd-command-consumer-grp");
				} catch (Exception e) {
					e.printStackTrace();
					running = false; // liveness may kill this app and restart it
	...
	private void generateErrorEvent(ShippingOrder shippingOrder,
		...
		ErrorEvent errorEvent = new ErrorEvent(new Date().getTime(), schemaVersion,
				shippingOrder.toShippingOrderPayload(), "Repository access issue");
		try {
			// Emit the event and consumer offsets as a transaction
